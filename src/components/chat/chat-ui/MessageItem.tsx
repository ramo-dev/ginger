...
export const MessageItem = ({ message, regenerate, status }: MessageItemProps) => {
    ...
    <div key={message.id}>
      {message.parts.map((m, index) => {
        return (
          ...
          >
            <MessageContent className={cn(role === "assistant" ? "w-full" : "w-fit")}>
              {(() => {
                if (m.type === "text") {
                  return (
                    <MessageResponse>
                ...
                if (m.type === "tool-createDocument") {
                  ...
                  return (
                    <DocumentTool
                      input={input as { title: string; content: string; fileType: "html" | "txt" | "md" | "docx" | "pdf" }}
                      output={output as { id: string; title: string; content: string; fileType: string; downloadUrl?: string; success: boolean } | undefined}
                      errorText={errorText}
                      state={state}
                      type={type}
                    />
      ...
      <div id={id} className={cn("my-2 flex gap-1", role === "user" ? "ml-auto justify-end" : "")}>
